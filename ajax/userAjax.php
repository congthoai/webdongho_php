<?php
    ...
    if(isset($_GET['idxoa'])){
        $rs = userModel::getInstance()->xoaUser($_GET['idxoa']);
        if($rs)
    ...
    $tongso = (int)userModel::getInstance()->getTongSoLuong();
...
<script> $j(".nInformation").toggle(4000);$('html,body').animate({
	scrollTop: 0
}, 'fast');
</script>
		...
		<form action="http://localhost/webphp/index.php/admin/user.html"
			...
			<table cellpadding="0" cellspacing="0" width="100%"
						      ...
						      if($p>1)
                                  for($i=1; $i<=$p; $i++){
                                     if(isset($_GET['page']) && $i == $_GET['page'])
                                         ...
                                         echo  "<a href='http://localhost/webdongho_php/ajax/userAjax.php?page=$i'>$i</a>";
                                  }
						      ?>
							</div>
						</td>
				...
				<tbody>
					...
					<?php 
					       include_once '../model/userModel.php';		
					       $listUser = userModel::getInstance()->getListUser($page, $limit);
					...
					<tr>
						...
						<td class="option">
							<a href="http://localhost/webdongho_php/ajax/update_userAjax.php?updateID=<?php echo $item['id']?>" title="Chỉnh sửa" class="tipS"> 
								<img src="../../image/crown/icons/color/edit.png" /> </a> 
								
							<a href="http://localhost/webdongho_php/ajax/userAjax.php?idxoa=<?php echo $item['id']?>" title="Xóa" class="tipXoa">
