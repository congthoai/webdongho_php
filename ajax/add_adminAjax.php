...
$flag = $username = $name = $email = $phone = $pass = $re_pass = "";
$flag="Cập nhật không thành công!";
$err_phone = $err_email = $err_name = $err_repass = $err_username="";
    ...
    $emailValid = adminModel::getInstance()->emailValid($_GET['email']);
    if ($emailValid == false)
        $err_email = "Email không hợp lệ";
    else
    $email = $_GET['email'];
    
    $checkUsername = registerModel::getInstance()->checkUsername($_GET['username']);
    if ($checkUsername)
        $err_username = "Username đã tồn tại!";
    
        if (! $checkUsername && $emailValid && $pass && $re_pass && $pass == $re_pass && $err_phone=="" && $name) {
        $rs = adminModel::getInstance()->addAdmin($name, $username, $pass, $phone, $email);
        if ($rs)
		...
		<form class="form" id="form" action="#"  method="get" name="addAdmin"
			...
			<fieldset>
				...
				<div class="formRow">
					...
					<div class="formRight">
						<span class="oneTwo"><input name="username" id="param_username"  value="<?php echo $username;?>" style="background-color: #ededc4;"
							_autocheck="true" type="text"></span> <span name="name_autocheck"
							class="autocheck"></span>
						<div name="name_error" class="clear error"><?php echo $err_username?></div>
					</div>
					<div class="clear"></div>
				</div>
				...
				<div class="formSubmit">
					<input type="submit" value="Thêm mới" class="redB" name="CapNhat"> 
				</div>
